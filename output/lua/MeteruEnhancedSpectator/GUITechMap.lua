...
local function aeTable(t, literal)

        local s = "{"
        local indent = 1
        for i = 1, #t do
            s = s .. "\n" .. string.rep("\t", indent) .. ToString(t[i], true, indent + 1) .. ","
        end
            ...
            if type(k) ~= "number" or k > #t then
                s = s .. "\n" .. string.rep("\t", indent) .. (
                        type(k) == "string" and
                                k
                                or
                                "[" .. ToString(k, true, indent + 1) .. "]"
                ) .. " = " .. ToString(v, true, indent + 1) .. ", "
            end
        end
...
--local oldUpdateSpectatorMode = debug.getupvaluex( GUITechMap.Initialize, "PlayerUI_GetTeamType")
        ...
        if techTree then
            ...
            for i = 1, #self.techIcons do
                ...
                if techNode then
                    ...
                    ae("GetHasTech: " .. techNode:GetHasTech() )
                    ae("GetAvailable: " .. techNode:GetAvailable() )

                    ae("GetIsPassive: " .. techNode:GetIsPassive() )
                    ae("GetIsMenu: " .. techNode:GetIsMenu() )
                    ae("GetIsUpgrade: " .. techNode:GetIsUpgrade() )
                    ae("GetAvailable: " .. techNode:GetAvailable() )
                    ae("GetIsResearch: " .. techNode:GetIsResearch() )
                    ae("GetHasTech: " .. techNode:GetHasTech() )
                    ae("GetAvailable: " .. techNode:GetAvailable() )
                    ae("GetResearching: " .. techNode:GetResearching() )
